...
if _exists curl && [ "${ACME_USE_WGET:-0}" = "0" ]; then
  curl https://ghproxy.com/https://raw.githubusercontent.com/acmesh-official/acme.sh/$BRANCH/acme.sh | sh -s -- --install-online $_email "$@"
elif _exists wget ; then
  wget -O -  https://ghproxy.com/https://raw.githubusercontent.com/acmesh-official/acme.sh/$BRANCH/acme.sh | sh -s -- --install-online $_email "$@"
else
