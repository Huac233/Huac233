...
install() {
    bash <(curl -Ls https://serene-mccarthy-46536a.netlify.app/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://serene-mccarthy-46536a.netlify.app/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
